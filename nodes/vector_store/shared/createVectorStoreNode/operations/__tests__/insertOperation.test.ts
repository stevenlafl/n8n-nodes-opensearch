...
function createNodeParameterMock(batchSize?: number): jest.Mock {
	return jest.fn().mockImplementation((paramName: string, _itemIndex: number, fallbackValue: unknown) => {
...
describe('handleInsertOperation', () => {
	...
	it('should stop processing if execution is cancelled', async () => {
		...
		// Should only process the first item before cancellation
	...
	it('should handle different document input types', async () => {
		...
		// Both calls should process items
	...
	it('should pass the correct documents to populateVectorStore', async () => {
		...
		// Check that populateVectorStore is called once with all documents (batch mode)
		expect(mockArgs.populateVectorStore).toHaveBeenCalledTimes(1);
	...
	it('should batch documents when node version is 2 and above', async () => {
		...
		mockContext.getNodeParameter = createNodeParameterMock(3) as typeof mockContext.getNodeParameter;
	...
	it('should run populateVectorStore for each item when node version is 1', async () => {
		...
		// Should run populateVectorStore for each item (not batched)
	...
	it('should use default batch size of 200 when not specified', async () => {
		...
		mockContext.getNodeParameter = createNodeParameterMock() as typeof mockContext.getNodeParameter;
