...
export async function handleRetrieveAsToolOperation<T extends VectorStore = VectorStore>(
	...
	// Note: We don't use extraArgs here to create a simple DynamicTool instead of DynamicStructuredTool
	// This avoids schema validation issues when the tool is called with a plain string
	const vectorStoreTool = createToolFromNode(node, {
		...
		func: async (query) => {
			...
			// Query is a plain string since we're using DynamicTool (no extraArgs)
