...
import type { ISupplyDataFunctions, INodeParameters } from 'n8n-workflow';
...
describe('handleRetrieveAsToolOperation', () => {
	...
	it('should create a tool with the correct name and description on version < 1.3', async () => {
	...
	it('should include metadata in results when includeDocumentMetadata is true', async () => {
		...
		const toolResult = await tool.func('test query') as Array<{ type: string; text: string }>;
